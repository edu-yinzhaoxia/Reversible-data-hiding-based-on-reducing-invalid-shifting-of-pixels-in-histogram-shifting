function [ complexity_I,local_complexity_I,x_posB,y_posB  ] = complexityB( I )
...
x_posB=zeros(m,n);%存放求取复杂度的像素值的横坐标位置
y_posB=zeros(m,n);%存放求取复杂度的像素值的纵坐标位置
for i=2:m-1
    for j=2:n-1
        if mod(i+j,2)~=0
...
for i=2:m-1
    for j=2:n-1
        if mod(i+j,2)~=0
            ...
            elseif i==511&&j==2%右上
                local_complexity_I(i,j)=complexity_I(i,j)+complexity_I(i-1,j+1);
            elseif i==2&&(j>=3&&j<=509)%左下、右下
                local_complexity_I(i,j)=complexity_I(i,j)+floor((complexity_I(i+1,j-1)+complexity_I(i+1,j+1))/2);
            elseif j==2&&(i>=3&&i<=509)%右上、右下
                local_complexity_I(i,j)=complexity_I(i,j)+floor((complexity_I(i-1,j+1)+complexity_I(i+1,j+1))/2);
            elseif j==511&&(i>=4&&i<=510)%左上、左下
                local_complexity_I(i,j)=complexity_I(i,j)+floor((complexity_I(i-1,j-1)+complexity_I(i+1,j-1))/2);
            elseif i==511&&(j>=4&&j<=510)%左上、右上
                local_complexity_I(i,j)=complexity_I(i,j)+floor((complexity_I(i-1,j-1)+complexity_I(i-1,j+1))/2);
            end
            x_posB(i,j)=i;
            y_posB(i,j)=j;
